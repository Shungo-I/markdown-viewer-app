    ...
    <div className={`${styles.markdownViewer} ${className || ''}`}>
      <ReactMarkdown
        ...
        components={{
          ...
          ul: ({ children }) => (
            <ul className={styles['markdownViewer-unorderedList']}>{children}</ul>
          ),
          ol: ({ children }) => (
          ...
          thead: ({ children }) => (
            <thead className={styles['markdownViewer-tableHead']}>{children}</thead>
          ),
          tbody: ({ children }) => (
            <tbody className={styles['markdownViewer-tableBody']}>{children}</tbody>
          ),
          th: ({ children }) => (
            <th className={styles['markdownViewer-tableHeaderCell']}>{children}</th>
          ),
          td: ({ children }) => (
          ...
          strong: ({ children }) => (
            <strong className={styles['markdownViewer-bold']}>{children}</strong>
          ),
          em: ({ children }) => (
            <em className={styles['markdownViewer-italic']}>{children}</em>
          ),
          del: ({ children }) => (
            <del className={styles['markdownViewer-strikethrough']}>{children}</del>
          ),
        }}
      >
        {content}
